...
sed -e 's/<\/a>//g' -e 's/^.*">//g' -e "s/ .*$2/, $2/g" -e 's/ [0-9]\{2\}:[0-9]\{2\}//g' -e "s/$2  /$2 0/g" -e "s/$/, ${typeevent}/g" ${typeevent}_tmp.txt > ${typeevent}_${2}.txt

  # Download the ads
  cat ${typeevent}_${2}.txt | cut -d, -f1 | while read line
  do
    echo $line
